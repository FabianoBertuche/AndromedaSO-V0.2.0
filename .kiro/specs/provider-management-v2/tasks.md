...
- [ ] 1. Adicionar método `delete` à interface `ProviderRepository`
...
- [ ] 2. Implementar `delete` no `ProviderRepositoryPostgres`
  - [ ] 2.1 Implementar o método `delete` em `provider.repository.postgres.ts`
  ...
  - [ ]* 2.2 Escrever teste de propriedade para `ProviderRepositoryMemory.delete` (Propriedade 1)
...
- [ ] 3. Implementar `delete` no `ProviderRepositoryMemory`
  - [ ] 3.1 Implementar o método `delete` em `provider.repository.memory.ts`
  ...
  - [ ]* 3.2 Escrever testes unitários para `ProviderRepositoryMemory.delete`
    - Caso: provider existente é removido com sucesso
    - Caso: provider inexistente lança `Error('Provider not found')`
    - _Requisitos: 1.4, 1.5_

- [ ] 4. Implementar `deleteProvider` no `ProviderOrchestratorService`
  - [ ] 4.1 Adicionar método `deleteProvider(id: string): Promise<void>` em `provider.orchestrator.service.ts`
    - Chamar `this.repository.findById(id)`, lançar `Error('Provider not found')` se `null`
    - Chamar `this.repository.delete(id)`
    - _Requisitos: 1.1, 1.2_
  - [ ]* 4.2 Escrever testes unitários para `ProviderOrchestratorService.deleteProvider`
    - Caso: delega ao repositório e retorna sem erro
    - Caso: propaga erro quando provider não encontrado
    - _Requisitos: 1.1, 1.2_

- [ ] 5. Adicionar rota `DELETE /api/providers/:id` em `providerRoutes.ts`
...
- [ ] 6. Checkpoint — verificar backend
...
- [ ] 7. Adicionar função `deleteProvider` em `kernel.ts`
...
- [ ] 8. Adicionar hook `useDeleteProvider` em `useProviders.ts`
...
- [ ] 9. Criar componente `DeleteConfirmationDialog`
...
- [ ] 10. Integrar botão Delete na tabela de providers em `ModelProviders.tsx`
...
- [ ] 11. Implementar detecção de tipo de provider no formulário (`authMode`)
...
- [ ] 12. Criar utilitários PKCE em `pkce.ts`
...
- [ ] 13. Implementar função `startOAuthFlow` em `ModelProviders.tsx`
  ...
  - Ler `client_id` de `import.meta.env.VITE_OPENAI_OAUTH_CLIENT_ID`
...
- [ ] 14. Instalar `react-router-dom` e configurar `BrowserRouter` no `App.tsx`
...
- [ ] 15. Criar componente `OAuthCallbackHandler`
...
- [ ] 16. Configurar variável de ambiente `VITE_OPENAI_OAUTH_CLIENT_ID`
...
- [ ] 17. Checkpoint final — verificar frontend e integração
