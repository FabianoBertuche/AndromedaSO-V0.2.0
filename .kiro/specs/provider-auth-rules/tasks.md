...
- [ ] 1. Adicionar `AuthMode` e `AuthConfig` em `frontend/src/types/model.ts`
...
- [ ] 2. Criar `frontend/src/utils/authConfig.ts` com a função `getAuthConfig`
  - Implementar o switch completo com os 20 providers conforme o design
  - Incluir fallback `default` retornando `mode: 'api-key'`

- [ ] 3. Criar `frontend/src/components/CredentialTooltip.tsx`
  - Componente simples: ícone `?` como link `<a>` abrindo em nova aba
  - Props: `{ url: string }`

## Fase 2: Backend

- [ ] 4. Estender `ProviderConfig` em `core/kernel/src/modules/providers/domain/entities/provider.entity.ts`
  - Adicionar `accessKeyId?: string` e `secretAccessKey?: string` ao tipo `ProviderConfig`

- [ ] 5. Atualizar `ProviderOrchestratorService.createProvider` em `core/kernel/src/modules/providers/services/providerOrchestratorService.ts`
...
- [ ] 6. Atualizar `frontend/src/pages/ModelProviders.tsx` — estados e imports
...
- [ ] 7. Atualizar o handler de troca de provider em `ModelProviders.tsx`
  - No `onChange` do `<select>`, limpar também `accessKeyId` e `secretAccessKey`
  - Remover a chamada `setAuthMode(getAuthMode(v)[0])`

- [ ] 8. Substituir renderização condicional dos campos em `ModelProviders.tsx`
...
- [ ] 9. Atualizar a função `addProvider` em `ModelProviders.tsx` com validação e novos campos
...
- [ ]* 10. Criar `frontend/src/utils/__tests__/authConfig.test.ts`
...
- [ ]* 11. Criar teste de round-trip AWS em `core/kernel/src/modules/providers/__tests__/`
